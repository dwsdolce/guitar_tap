...
<style>
  ...
  ul         { margin: 4px 0; padding-left: 20px; }
    ...
    parts.append(_row(
        ...
        "uses an adaptive noise-floor threshold for brace tap detection and always picks "
        "the strongest peak."
    ))
    parts.append(_row(
        "Results",
    ...
    parts.append(_row(
        ...
        "Import Measurement to load a .guitartap file from disk or another device. "
        "Use Export All to write your whole library to a single .guitartap file "
        "&mdash; for backing it up or moving the complete set of measurements to "
        "another platform; re-import it with Import Measurement. Use "
    ...
    parts.append(_row(
        ...
        "(higher). Displayed in dB. In Plate and Brace mode the threshold is relative "
        "&mdash; it sets the headroom above an adaptive noise floor estimate, so the "
        "trigger adapts to the ambient noise level."
    ))
    parts.append(_row(
        "Peak Min (slider)",
