...
class GuitarMode(Enum):
    ...
    def peaks_in_display_range(
        ...
        overridden_peak_ids: set = frozenset(),
        guitar_type: "GuitarType | None" = None,
    ) -> list:
        ...
        **A peak the user has named is known.** The unknown-mode filter is positional -- is_known()
        (*does the frequency fall in a band*) -- **except** for peaks carrying a manual mode label,
        which are always kept.  Pass those in ``overridden_peak_ids``.

        The positional test alone is equivalent to the analyzer's assigned-mode test for every peak
        *without* an override, because classify_all() falls back to a per-frequency lookup for peaks
        its one-per-mode claiming pass did not take.  The two differ only under a user override.

        Note: this function previously applied the positional test unconditionally, on the argument
        that a chart layer should be positional.  That was wrong -- it discarded the user's own
        identification, so a peak labelled "Wolf note" lost its results row while keeping its dot (or
        lost both, if it sat outside every band).  One predicate now governs the results panel, this
        dot layer and the annotation badges -- see TapToneAnalyzer.is_unknown(), of which this is the
        static, analyzer-free form.
        ...
        - Parameters:
          ...
          - overridden_peak_ids: Peaks the user has given a manual mode label.  Kept regardless of
            position.  A set rather than a closure or an analyzer reference so this stays static and
            pure, and ports across platforms unchanged.
          - guitar_type: The guitar type whose mode-range bands are used.

        Mirrors Swift GuitarMode.peaksInDisplayRange(_:minFreq:maxFreq:isGuitar:showUnknownModes:overriddenPeakIDs:guitarType:).
        """
        ...
        return [
            p for p in in_range
            if p.id in overridden_peak_ids or GuitarMode.is_known(p.frequency, guitar_type)
        ]
