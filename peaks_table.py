...
class PeakTable(QtWidgets.QWidget):
    ...
    def __init__(self) -> None:
        ...
        self.peaks_table.sortByColumn(
            pm.ColumnIndex.Freq.value, QtCore.Qt.SortOrder.AscendingOrder
        )
        self.peaks_table.resizeColumnsToContents()
        self.peaks_table.setColumnWidth(
            self.model.modes_column, self.model.modes_width * 8
        )
        self.peaks_table.setSelectionBehavior(
            QtWidgets.QTableView.SelectionBehavior.SelectRows
        )
        self.peaks_table.setSelectionMode(
        ...
        self.peaks_table.setItemDelegateForColumn(
            self.model.modes_column, self.mode_delegate
        )
        self.show_delegate = sbd.ShowComboDelegate(self)
        self.peaks_table.setItemDelegateForColumn(
            self.model.show_column, self.show_delegate
        )
        self.peaks_table.setEditTriggers(
            QtWidgets.QTableView.EditTrigger.AllEditTriggers
        )
        self.peaks_table.setToolTip(
            "Displays the peaks found. When results are held, select\n"
        ...
        save_peaks.setToolTip(
            "Save the captured peaks and related information to a file"
        )
            ...
            try:
                with open(filename, "w", encoding="utf-8-sig") as csvfile:
                    ...
                    for row in range(data_model.rowCount(QtCore.QVariant())):
                        writer.writerow(
                            data_model.data_value(data_model.index(row, column))
                            for column in columns
                        )
            except Exception as e:
                QtWidgets.QMessageBox.warning(
                    self, "Error in saving peaks", f"Table was not saved\n{str(e)}"
                )
    ...
    def select_row(self, freq: int) -> None:
        ...
        if freq_index >= 0:
            ...
            proxy_freq_index: QtCore.QModelIndex = proxy_model.mapFromSource(
                data_freq_index
            )
