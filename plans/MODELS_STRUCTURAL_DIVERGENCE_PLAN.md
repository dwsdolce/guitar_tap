...
- [x] 18. **WI-17** (auto-device-switch on plug-in) — `_on_devices_refreshed()` in `tap_tone_analyzer_control.py` now: (1) flushes PortAudio's cached device list via `sd._terminate()`/`sd._initialize()`; (2) calls `mic.load_available_input_devices()` with callback-suppression (sets `mic._on_devices_changed = None` temporarily to prevent re-entry); this routes through `_auto_select_on_hotplug()` — the fingerprint-based diff + aggregate filter + built-in fallback logic — mirroring Swift's `loadAvailableInputDevicesMacOS()`; (3) calls `set_device(new_device)` if auto-selection changed the selection; (4) emits `currentDeviceLost` if the previous device name is no longer in the new list. View `_on_devices_changed` and `_on_device_lost` simplified to read `mic.selected_input_device` from model (no longer re-implement selection logic or call `set_device()`). 314 tests green.
...
- [x] 21. **WI-24** (dimension getter inline defaults) — added 11 `DEFAULT_*` class constants and `v if v > 0 else cls.DEFAULT_*` fallbacks to all 11 dimension getters in `TapDisplaySettings`. Removed the duplicate defaults that were previously hardcoded in `AppSettings` dimension getters (those now return raw `0.0` when unset, mirroring `UserDefaults.float(forKey:)`). Updated all direct `AppSettings` dimension reads in `tap_tone_analysis_view.py` (both UI population and save-dialog fallbacks) to read through `TapDisplaySettings` instead. `TapDisplaySettings` is now the single source of truth for dimension defaults — matching Swift's structure exactly. 314 tests green.
- [x] 22. **WI-25** (`minFrequency`/`maxFrequency` convenience properties) — split into distinct methods matching Swift's structure and names exactly: `min_frequency_for(meas_type)` / `max_frequency_for(meas_type)` mirror `minFrequency(for:)` / `maxFrequency(for:)`; `min_frequency()` / `max_frequency()` mirror Swift's no-arg `static var` getter; `set_min_frequency(v)` / `set_max_frequency(v)` mirror the `static var` setter; `set_min_frequency_for(v, meas_type)` / `set_max_frequency_for(v, meas_type)` mirror `setMinFrequency(_:for:)` / `setMaxFrequency(_:for:)`. Updated callers in `tap_display_settings.py`, `tap_tone_analyzer_measurement_management.py`, and `test_wi6_tap_display_settings.py`. 314 tests green.
- [x] 23. **WI-26** (`reset_to_defaults` per-type frequency keys) — `reset_to_defaults` / `resetToDefaults` was dead code in both Python and Swift (never called from anywhere). Removed from `tap_display_settings.py`, `TapDisplaySettings.swift`, and the 3 Python unit tests in `test_wi6_tap_display_settings.py`. `fft_canvas.py` structural divergence also resolved: `_reset_both_to_saved` now calls `TapDisplaySettings.min_frequency()` / `max_frequency()` (no-arg, mirrors Swift's `TapDisplaySettings.minFrequency` static var); `_reset_freq_to_defaults` calls `TapDisplaySettings.default_min_frequency(TapDisplaySettings.measurement_type())` / `default_max_frequency(measurement_type())` (mirrors Swift's explicit `type = TapDisplaySettings.measurementType` read). 311 tests green.
- [x] 24. **WI-3, WI-4, WI-5, WI-8, WI-9** — documentation passes; WI-4 (`tap_tone_measurement.py` D28/D29) and WI-8 (`spectrum_snapshot.py` D21) were already complete. WI-3 (`resonant_peak.py`): `id` and `timestamp` field comments updated to concise canonical form (`# str form of UUID — Swift uses UUID type` / `# ISO-8601 string — Swift uses Date`); `mode_label` comment updated to `# Python-only display hint — not present in Swift ResonantPeak. Set by the view layer before rendering; not persisted.` WI-5 (`material_properties.py`): added unit storage note to module docstring (`NOTE: stores dimensions in mm and mass in g (user-facing units). Swift equivalent stores in SI (m, kg) directly. Use .length_m, .width_m, .thickness_m, .mass_kg for SI access.`). WI-9 (`microphone_calibration.py`): extended `_DEVICE_MAP_KEY` comment to explain PortAudio limitation vs CoreAudio UID; `audio_device.py` `fingerprint` docstring was already complete. 311 tests green.
- [x] 25. **WI-19** (platform-native rate-change listeners) — macOS: `_get_coreaudio_device_id()` enumerates CoreAudio device IDs by name; `_start_coreaudio_sample_rate_monitor(device_name)` registers `kAudioDevicePropertyNominalSampleRate` (0x6E737274) via `AudioObjectAddPropertyListener` on the matched device; `_stop_coreaudio_sample_rate_monitor()` removes it. Windows/Linux: 5-second `sd.query_devices()` polling fallback via daemon thread (`_start_windows/linux_sample_rate_monitor()` / `_stop_*`). `_close_stream_only()` stops the listener before closing the stream (mirrors Swift `stop()` → `unregisterSampleRateListener()`). `set_device()` re-registers after opening the new stream. `_stop_hotplug_monitor()` also calls the appropriate stop. `RealtimeFFTAnalyzer.__init__` accepts `on_sample_rate_changed` callable (stored as `self._on_sample_rate_changed`) and registers the initial listener at startup. Handler `_on_mic_sample_rate_changed()` added to `TapToneAnalyzerControlMixin`: dispatches `set_device(selected_input_device)` onto the Qt main thread via `QTimer.singleShot(0, ...)` (0.3 s delay already applied by listener thread). Mirrors Swift `registerSampleRateListener(for:)` + `stop()` / `try start()` pattern. Linux uses polling (ALSA has no push-event model for rate changes). 311 tests green.
- [x] 26. **WI-20** (menu bar comment + shortcut fix) — replaced single-line comment with a full block explaining the intentional divergence from Swift's AppCommands (App/File/View/Edit/Help menus via CommandGroup) and mapping each Swift shortcut to its Python toolbar/context-menu equivalent. Also fixed: `QKeySequence("Ctrl+?")` replaced with `QKeySequence(QKeySequence.StandardKey.HelpContents)` — on macOS Qt resolves `Ctrl` as the Control key (not Command), so the old shortcut was dead; `HelpContents` resolves to Cmd+? on macOS and F1 on Windows/Linux. Cmd+? correctly focuses the Help menu (same behaviour as Swift's AppCommands) — this is the standard macOS convention; no keyboard shortcut directly opens the Help window in either app. 311 tests green.
- [x] 27. **WI-21** (Python menu bar + help content update) — added GuitarTap, File, View, and Help menus to `MainWindow.__init__` in `tap_tone_analysis_view.py`, mirroring Swift's `AppCommands` structure exactly. Settings… (⌘,/Ctrl+,) uses `QAction.MenuRole.PreferencesRole` so it appears in the macOS Application menu automatically. File menu: Save Measurement… (⌘S), Export Spectrum Image… (⌘E), Export PDF Report… (⇧⌘E) — disabled until measurement complete, synced with toolbar buttons at all three `setEnabled` call sites. View menu: Auto dB (⌘0, checkable, synced with toolbar toggle), Cycle Annotations (⌘`), Show Metrics (⌘M), Show Measurements (⌘L). Help menu: Guitar Tap Help (⌘?/F1). Help content in `help_view.py` updated to describe the actual four-menu structure. 311 tests green.
...
- [x] 16. **WI-17:** Plug in a new microphone while the Python app is running — verify it auto-switches without user interaction. Unplug a device — verify the app does not crash and does not attempt to auto-switch. ✓ User verified.
...
- [x] 17c. **WI-24:** On a machine with existing settings saved (i.e. values > 0 already stored): open Settings → Plate/Brace tab — verify all dimension fields show the previously saved values (not the hardcoded defaults). To simulate a fresh install, clear QSettings with `defaults delete com.dolcesfogato.guitar_tap` (plist at `~/Library/Preferences/com.dolcesfogato.guitar_tap.plist`), then relaunch: open Settings → Plate/Brace tab — verify dimension fields show the correct defaults (plate 500 × 200 × 3 mm / 100 g; body 490 × 390 mm; brace 300 × 6 × 12 mm / 8 g; custom f_vs 75). Open Plate mode, perform a tap — verify Analysis Results panel shows the correct Gore target thickness (i.e. `guitar_body_length` and `guitar_body_width` are non-zero). Export a PDF — verify plate/brace dimensions in the report are non-zero. Save a measurement, reload it — verify plate/brace dimensions round-trip correctly.
- [x] 17d. **WI-25:** Code-verified: `min_frequency()` / `max_frequency()` are the no-arg methods mirroring Swift's `static var minFrequency/maxFrequency`; `min_frequency_for(meas_type)` / `max_frequency_for(meas_type)` mirror `minFrequency(for:)` / `maxFrequency(for:)`. Structure and names now match Swift exactly. 314 tests green.
- [x] 17e. **WI-26:** Code-verified: `reset_to_defaults` / `resetToDefaults` removed from both codebases and test suite (dead code). `fft_canvas.py` `_reset_both_to_saved` uses `_tds.min_frequency()` / `_tds.max_frequency()` (no-arg, reads from settings store via current measurement type — mirrors Swift's `TapDisplaySettings.minFrequency` static var). `_reset_freq_to_defaults` uses `_tds.default_min_frequency(_tds.measurement_type())` (reads type from settings store — mirrors Swift's `let type = TapDisplaySettings.measurementType`). 311 tests green. Runtime: right-click spectrum plot → "Reset Both Axes to Saved" — frequency axis snaps to saved min/max for the current measurement type. "Reset Frequency Axis to Defaults" — frequency axis snaps to the default range for the current type (e.g. 20–1200 Hz for Plate, 20–5000 Hz for Guitar). Switching measurement type and repeating both — each type uses its own saved/default frequencies independently. ✓ User verified.
...
- [x] 20. **WI-20:** Comment block confirmed present in `tap_tone_analysis_view.py`. Cmd+? correctly focuses the Help menu in both the Python and Swift apps — this is the standard macOS convention (Swift's `CommandGroup(replacing: .help)` puts the item in the Help menu; Cmd+? focuses that menu; the Help window opens by clicking the menu item, not via a keyboard shortcut). Behaviour is identical in both apps; the implementation and comment are correct. ✓ User verified.
- [ ] 21. **WI-21:** Launch the Python app and verify the four menus appear in the menu bar:
  1. **GuitarTap menu** (on macOS this appears as the application name menu at top-left): contains Settings… (⌘,). Click it — verify the Settings dialog opens, same as the toolbar Settings button.
  2. **File menu**: Save Measurement… (⌘S), Export Spectrum Image… (⌘E), Export PDF Report… (⇧⌘E). Before any tap these should be disabled (greyed out). After a tap completes, all three should become enabled. Verify each shortcut triggers the correct action.
  3. **View menu**: Auto dB (⌘0, checkable — should show a checkmark when active and keep the toolbar Auto dB button in sync), Cycle Annotations (⌘`, should cycle the annotation overlay), Show Metrics (⌘M), Show Measurements (⌘L).
  4. **Help menu**: Guitar Tap Help (⌘?/F1) — opens the Help dialog.
  5. Open Help > Guitar Tap Help — verify the Controls Reference "Menu Bar" row describes the four-menu structure (GuitarTap, File, View, Help) with the correct shortcuts.
- [ ] 21a. **WI-19:** Runtime verification — steps differ by platform:
  - **macOS** (CoreAudio native listener, ~0.5 s response):
    1. Start the Python app; note the sample rate in Settings (e.g. 44100 Hz).
    2. Open **Audio MIDI Setup** (Applications > Utilities). Select the active input device (the one shown in the Python app's Settings drop-down). Change its sample rate (e.g. 44100 → 48000 Hz).
    3. The Python app should automatically restart the audio stream within ~0.5 s. Tap detection should continue to work; the stream should not freeze or crash.
    4. Change Audio MIDI Setup back to the original rate; verify the app restarts again.
    5. Negative test: if `_get_coreaudio_device_id` cannot match the device name (e.g. a USB device whose CoreAudio name differs slightly from its PortAudio name), the listener will silently not register — the app continues to work but won't auto-restart on rate change. Check the Python process does not crash in this case.
  - **Windows** (5 s polling fallback):
    1. Start the Python app. Open **Sound Settings** (right-click the speaker icon → Sound settings → More sound settings → Recording tab). Double-click the active input device → Advanced tab. Note the current Default Format sample rate.
    2. Change the Default Format to a different sample rate (e.g. 44100 Hz → 48000 Hz) and click OK.
    3. Wait up to ~10 s (one full poll interval). The Python app should automatically restart the audio stream; tap detection should continue.
    4. Change the Default Format back to the original rate; verify the app restarts again within ~10 s.
    5. Negative test: if `sd.query_devices(device_index)` fails (device temporarily unavailable during format change), the polling thread silently catches the exception and retries on the next interval — verify the app does not crash.
  - **Linux** (5 s polling fallback — PulseAudio / PipeWire systems):
    1. Start the Python app. Identify the active input device name (shown in Settings).
    2. Use `pactl` to change the source sample rate: `pactl set-source-volume @DEFAULT_SOURCE@ 100%` is not the right command — instead use `pacmd set-default-source <name>` after loading a different-rate profile, or use `pavucontrol` to switch the active source to a different profile/rate. Alternatively, use `pw-metadata` (PipeWire) or edit `/etc/pulse/daemon.conf` to change `default-sample-rate` and restart PulseAudio (`pulseaudio -k`).
    3. Wait up to ~10 s. The Python app should restart the audio stream.
    4. On pure ALSA systems (no PulseAudio / PipeWire): the polling fallback calls `sd.query_devices()` which reads the ALSA-level `default_samplerate`; this value is unlikely to change because ALSA has no mid-session rate-change model. The stream will not be restarted in this case — this is the expected behaviour and is documented in the implementation. Verify the app does not crash.
...
- [x] 23. **WI-35:** (Code-verified: no `self.measurement_type_combo`, `self.guitar_type_combo`, or `self._current_mt()` call sites remain in `tap_tone_analysis_view.py`; `_on_guitar_type_changed` deleted; 311 tests green.) Runtime checks:
  1. Start the app in **Acoustic Guitar** mode. Open Settings → change to **Classical Guitar** → Apply. Confirm the dashed mode-boundary lines on the spectrum plot shift to the Classical ranges (Air mode lower bound moves from ~100 Hz to ~80 Hz). Confirm peaks are reclassified. ✓ User verified.
  2. Open Settings → change to **Plate** → Apply. Confirm mode-boundary lines disappear entirely (no dashed lines; plate mode has no guitar bands). ✓ User verified.
  3. Open Settings → change back to **Acoustic Guitar** → Apply. Confirm mode-boundary lines reappear with Acoustic ranges. ✓ User verified.
  4. Quit and relaunch. Confirm the measurement type on relaunch matches what was set in step 3 (persistence check). ✓ User verified.
  5. Perform a tap in Guitar mode and confirm the result appears in the Guitar Analysis Results panel. Switch to Plate mode, perform a tap, confirm the result appears in the Plate Properties panel. ✓ User verified.
