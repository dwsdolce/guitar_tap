...
- [x] 22. **WI-25** (`minFrequency`/`maxFrequency` convenience properties) — split into distinct methods matching Swift's structure and names exactly: `min_frequency_for(meas_type)` / `max_frequency_for(meas_type)` mirror `minFrequency(for:)` / `maxFrequency(for:)`; `min_frequency()` / `max_frequency()` mirror Swift's no-arg `static var` getter; `set_min_frequency(v)` / `set_max_frequency(v)` mirror the `static var` setter; `set_min_frequency_for(v, meas_type)` / `set_max_frequency_for(v, meas_type)` mirror `setMinFrequency(_:for:)` / `setMaxFrequency(_:for:)`. Updated callers in `tap_display_settings.py`, `tap_tone_analyzer_measurement_management.py`, and `test_wi6_tap_display_settings.py`. 314 tests green. Note: `fft_canvas.py` `_reset_both_to_saved` and `_reset_freq_to_defaults` still call `AppSettings.f_min/f_max` directly — addressed under WI-26.
...
- [x] 17d. **WI-25:** Code-verified: `current_min_frequency()` / `current_max_frequency()` are now distinct no-arg methods mirroring Swift's `static var minFrequency/maxFrequency`; `min_frequency(meas_type)` / `max_frequency(meas_type)` mirror `minFrequency(for:)` / `maxFrequency(for:)`. Structure now matches Swift exactly. 314 tests green.
