...
**D45 (resolved)** — `startTapSequence` triggering layer: Swift's `startTapSequence()` is called from model-layer code and owns `isMeasurementComplete = false` internally. Python's `start_tap_sequence()` existed in the model but was bypassed by `canvas.start_analyzer()`, which set `analyzer.set_measurement_complete(False)` and cleared `captured_taps` directly from the view layer. Fixed: `start_tap_sequence()` now sets `is_measurement_complete = False` and emits `measurementComplete(False)` itself; `canvas.start_analyzer()` now calls `analyzer.start_tap_sequence()` and no longer reaches into model state directly.
...
| D32 | macOS menu bar | Full native menu bar via `AppCommands`: GuitarTap menu (Settings… ⌘,), File menu (Save ⌘S, Export Spectrum ⌘E, Export PDF ⇧⌘E), View menu (Auto dB ⌘0, Cycle Annotations ⌘\`, Show Metrics ⌘M, Show Measurements ⌘L), Help menu (search + Help window) | Python has a menu bar with equivalent items under File and Help menus, but keyboard shortcuts and menu structure differ; no View menu grouping; Settings is under File not a dedicated app menu | Low — platform conventions differ legitimately; Python menu bar already functional; document the intentional differences |
| D33 | In-app help content | `HelpView.swift` — static SwiftUI List with `helpRow` entries; iOS toolbar layout rows describe iPhone/iPad button placement including new Help (?) button; macOS-only rows describe the menu bar (GuitarTap, File, View, Help menus with shortcuts); generated into `GuitarTap-Quick-Start-Guide.md/.pdf` via `generate-help.sh` | `help_view.py` — HTML built by `_build_help_html()`; Controls Reference section describes toolbar controls but has no platform-conditional toolbar layout rows and no menu bar reference section; docs/GuitarTap-Quick-Start-Guide.html/.pdf generated separately | Medium — help content has drifted; Python help lacks: (1) toolbar layout descriptions, (2) menu bar reference, (3) Help (?) button mention; Python docs generation process is separate from Swift's `generate-help.sh` |
| D45 | `startTapSequence` triggering layer | `startTapSequence()` is a model method; owns `isMeasurementComplete = false` | `canvas.start_analyzer()` (view) bypassed `start_tap_sequence()`, set `set_measurement_complete(False)` and cleared `captured_taps` directly | Low — **resolved**: `start_tap_sequence()` now owns `is_measurement_complete`; canvas delegates entirely to model |
| D44 | `_finish_capture` — `selectedPeakIDs` and `identifiedModes` after averaged tap | Swift `processMultipleTaps()` (lines 804-812 of `TapToneAnalyzer+SpectrumCapture.swift`) sets `selectedPeakIDs = guitarModeSelectedPeakIDs(from: peaksFromAveragedSpectrum)` (guitar-mode auto-selected subset) then `identifiedModes = peaksFromAveragedSpectrum.map { (peak: $0, mode: captureModeMap[$0.id] ?? .unknown) }` (full classify-all map) | Python `_finish_capture` set `selected_peak_ids = {p.id for p in peaks}` (all peaks — fixed UUID staleness bug but does not match Swift's guitar-mode-subset selection); `identified_modes` is not set at all — left as whatever `analyze_magnitudes` last computed from the live pre-capture input, so it references old peak objects with stale UUIDs | Medium — `selected_peak_ids` and `identified_modes` are out of sync with the new post-average peak objects after capture completes |
| D46 | `measurementType` — single source of truth | Swift: `TapDisplaySettings.measurementType` (backed by `UserDefaults`) is the **only** source of truth. The Settings view holds `@State var selectedMeasurementType` as a transient local copy during editing; on Apply it writes back to `TapDisplaySettings`. All other code — analyzer, tap detection, canvas — reads directly from `TapDisplaySettings`. No secondary copy exists at runtime. | Python: **four concurrent copies** in flight: (1) `self.measurement_type_combo` combo text (view state); (2) `AppSettings`/`TapDisplaySettings` (QSettings — what the model reads); (3) `self.analyzer._measurement_type` instance var (set via `fft_canvas.set_measurement_type()`); (4) `fft_canvas` reads QSettings independently on some code paths. The combo is the interaction surface; it writes to QSettings via `_on_measurement_type_changed()` which fires on combo change or on explicit Settings Apply. Divergence can occur if: (a) `setCurrentText()` fires no signal because the text didn't change; (b) the analyzer's local copy is stale before `set_measurement_type()` is called. Recent fixes (`_init_state` unconditional write, `_apply_settings` unconditional write) close the known gap at startup and on Settings Apply. The routing-critical code paths (tap detection, `start_tap_sequence`) already read from `TapDisplaySettings` directly, so the analyzer's local copy only affects post-capture peak classification where the type is always consistent by that point. | Medium — **partially mitigated** by existing fixes. Full unification to match Swift's pattern would require replacing Qt combo widgets as the interaction surface (significant UI refactor) for marginal safety gain. Deferred. |
