...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.saved_path: str = ""
        with open(
            os.path.join(basedir, "./version"), "r", encoding="UTF-8"
        ) as file_handle:
        ...
        self.peaks_controls.min_spin.valueChanged.connect(
            self.plot_controls.fmin_changed
        )
        self.peaks_controls.max_spin.valueChanged.connect(
            self.plot_controls.fmax_changed
        )
        ...
        self.plot_controls.fft_canvas.peakDeselected.connect(
            self.peak_widget.clear_selection
        )
        self.plot_controls.fft_canvas.averagesChanged.connect(
            self.peaks_controls.set_avg_completed
        )
        self.plot_controls.fft_canvas.framerateUpdate.connect(
            self.peaks_controls.set_framerate
        )
    ...
    def show_device_dialog(self, _) -> None:
        ...
        dlg = SD.ShowInputDevices(self.plot_controls.fft_canvas.get_py_audio())
        dlg.exec()
